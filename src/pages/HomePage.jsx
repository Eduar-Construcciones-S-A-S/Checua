      ...
      <div className="w-full max-w-xl space-y-6">
        {!isModalOpen && (
          <div className="flex justify-start">
            <button onClick={onOpenModal} className="flex items-center gap-2 px-4 py-2 text-xs font-bold text-brand-primary hover:text-brand-dark transition-colors group">
              <svg className="w-4 h-4 transition-transform group-hover:-translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>
        ...
        <div className="mb-5 sm:mb-8 md:mb-12 px-2">
          <div className="max-w-3xl mx-auto relative">
            <div className="flex items-center justify-between z-10 relative">
              {[{ n: 1, label: t('steps.step1_title') || 'Plan & Cliente' },{ n: 2, label: t('steps.step2_title') || 'Participantes' },{ n: 3, label: t('steps.step3_title') || 'Resumen' }].map((step, i, arr) => {
                const isActive = currentStep === step.n;
                const isCompleted = currentStep > step.n;
                return (
                  <div key={step.n} className="flex flex-col items-center gap-2 flex-1 relative">
                    <button type="button" onClick={() => { if (step.n === 1) { setCurrentStep(1); setShowSummary(false); } else if (step.n === 2 && currentStep >= 2) { setCurrentStep(2); setShowSummary(false); setShowCompanionsSection(true); } else if (step.n === 3 && currentStep === 3) { setCurrentStep(3); } }} disabled={step.n > currentStep} className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center font-black text-sm md:text-base border-2 transition-all duration-300 z-10 ${isCompleted ? 'bg-brand-primary border-brand-primary text-white shadow-lg scale-105' : isActive ? 'bg-white dark:bg-dark-bg-card border-brand-primary text-brand-primary shadow-xl ring-4 ring-brand-primary/20 scale-110' : 'bg-white/50 dark:bg-dark-bg-card/50 border-brand-border/50 dark:border-dark-border/50 text-brand-text-secondary/40 dark:text-dark-text-secondary/40 cursor-not-allowed'} ${step.n <= currentStep ? 'cursor-pointer' : 'cursor-not-allowed'}`}>
                      {isCompleted ? <svg className="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg> : step.n}
                    </button>
                    <p className={`text-[10px] md:text-xs font-black uppercase tracking-wider text-center max-w-[120px] transition-colors ${isActive || isCompleted ? 'text-brand-primary dark:text-brand-primary' : 'text-brand-text-secondary/40 dark:text-dark-text-secondary/40'}`}>{step.label}</p>
                    <span className={`text-[8px] md:text-[9px] uppercase tracking-widest text-center ${isActive ? 'text-brand-primary/60 dark:text-brand-primary/60 font-black' : 'text-brand-text-secondary/30 dark:text-dark-text-secondary/30'}`}>{isActive ? t('steps.step_active') || 'Actual' : ''}</span>
                    {i < arr.length - 1 && <div className={`hidden md:block absolute top-5 -right-1/2 w-full h-[3px] -z-0 ${currentStep > step.n ? 'bg-brand-primary shadow-[0_0_10px_rgba(132,204,22,0.6)]' : 'bg-brand-border/40 dark:bg-dark-border/40'}`}></div>}
                  </div>
                );
              })}
            </div>
        ...
        {currentStep === 1 && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
            ...
            <div className="pt-4 grid gap-3 md:grid-cols-2">
              <button onClick={handleStep1AddCompanions} className="rounded-[1.25rem] px-6 py-4 md:py-5 font-black text-sm md:text-base uppercase tracking-wider border-2 border-brand-primary/40 text-brand-primary bg-brand-primary/5 dark:bg-brand-primary/10 hover:bg-brand-primary/15 hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2">{t('steps.step1_add_companion') || 'Añadir acompañante'}</button>
              <button onClick={handleStep1ReserveAlone} className="btn-animate-continue w-full"><span className="text_button">{t('steps.step1_reserve_alone') || 'Reservar'}</span></button>
            </div>
          </div>
        )}
        ...
        {currentStep === 2 && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
            ...
            <div className="pt-2 grid gap-3 md:grid-cols-2"><button onClick={() => setCurrentStep(1)} className="rounded-[1.25rem] px-6 py-4 md:py-5 font-black text-sm md:text-base uppercase tracking-wider border-2 border-brand-primary/20 text-brand-primary">{t('steps.back_step1') || 'Volver al plan'}</button><button onClick={handleAddCompanions} className="rounded-[1.25rem] px-6 py-4 md:py-5 font-black text-sm md:text-base uppercase tracking-wider border-2 border-brand-primary/40 text-brand-primary">{t('steps.add_participant') || 'Añadir participante'}</button></div>
            <button onClick={handleStep2Continue} className="btn-animate-continue w-full mt-2"><span className="text_button">{t('steps.step2_continue') || 'Continuar con la reserva'}</span></button>
          </div>
        )}

        {showSummary && (
          <div id="reservation-summary" className="animate-in fade-in slide-in-from-bottom-4 duration-500 pt-10">
            <div className="card-premium p-6 md:p-10 space-y-8">
              <div className="card-accent-line"></div>
              <div className="text-center relative pt-4"><h3 className="text-2xl md:text-3xl font-black text-brand-text-main dark:text-dark-text-main uppercase tracking-tight">{t('summary.title')}</h3><div className="h-1.5 w-16 bg-brand-primary mx-auto mt-3 rounded-full"></div></div>
              <div className="grid gap-8">
                <div className="space-y-4"><p className="section-title-premium !ml-0">{t('summary.responsible')}</p><div className="bg-brand-light/30 dark:bg-dark-bg-main/30 rounded-[2rem] p-6 border border-brand-primary/10"><p className="text-brand-text-main dark:text-dark-text-main font-black text-lg md:text-xl mb-3 break-words whitespace-normal">{reservationData.contact.nombre_jefe_reserva}</p></div></div>
                {reservationData.companions.length > 0 && <div className="space-y-4 pt-4 border-t border-brand-light dark:border-dark-border"><p className="section-title-premium !ml-0">{t('summary.registered_companions')} ({reservationData.companions.length})</p></div>}
                <div className="space-y-8">
                  <div className="space-y-4"><p className="section-title-premium !ml-0">{t('summary.experience')}</p><div className="bg-brand-light/30 dark:bg-dark-bg-main/30 rounded-[2rem] p-6 border border-brand-primary/10 space-y-5"><p className="text-brand-text-main dark:text-dark-text-main font-black text-base sm:text-lg md:text-xl"><span className="mr-2">{getPlanEmoji(reservationData.tour.tour_reserva)}</span><span className="uppercase">{reservationData.tour.tour_reserva}</span></p><div className="rounded-[1.5rem] border-2 border-brand-primary/30 bg-brand-primary/10 p-5 text-center"><p className="text-[10px] sm:text-xs font-black text-brand-primary uppercase">ABONO MÍNIMO PARA CONFIRMAR</p><p className="mt-2 text-2xl sm:text-3xl font-black text-brand-primary">{formatCOP(depositAmount)}</p></div><div className="space-y-3 rounded-[1.5rem] bg-white/50 dark:bg-dark-bg-card/40 border border-brand-primary/10 p-5"><p className="text-[10px] font-black uppercase">RESUMEN DEL PAGO</p><p className="text-2xl sm:text-3xl font-black">{formatCOP(totalPrice)}</p><p>Saldo para el día del evento: {formatCOP(remainingAmount)}</p></div></div></div>
                  <div className="space-y-4"><p className="section-title-premium !ml-0">{t('summary.date_time')}</p><div className="bg-brand-light/30 dark:bg-dark-bg-main/30 rounded-[2rem] p-6 border border-brand-primary/10 space-y-3"><p className="text-brand-text-main dark:text-dark-text-main font-black text-sm sm:text-base md:text-lg capitalize">{reservationData.date.rawDate?.toLocaleDateString(i18n.language === 'en' ? 'en-US' : 'es-ES', { weekday: 'long', day: 'numeric', month: 'long' })}</p>{reservationData.time?.label ? <div className="flex items-baseline gap-2"><p className="text-brand-primary font-black text-xl md:text-2xl">{formatTime12h(reservationData.time.label)}</p><span className="text-[10px] font-black uppercase">{getMeridiem(reservationData.time.label)}</span></div> : null}</div></div>
                </div>
              </div>
              <div className="flex flex-col sm:flex-row gap-4 pt-4"><button onClick={handleEditInformation} className="flex-1 py-4 px-6 border-2 border-brand-border dark:border-dark-border rounded-full">{t('steps.edit_all') || t('summary.edit')}</button><button onClick={() => { setShowSummary(false); setCurrentStep(2); setShowCompanionsSection(true); }} className="flex-1 py-4 px-6 bg-brand-light dark:bg-dark-bg-main border-2 border-brand-primary/20 rounded-full">{t('steps.edit_participants') || t('summary.add_companions')}</button></div>
              <div className="pt-6 border-t border-brand-light dark:border-dark-border mt-8"><button onClick={handleProceedToPayment} disabled={isSaving} className={`btn-animate-continue w-full !bg-brand-primary ${isSaving ? 'opacity-70 cursor-not-allowed' : ''}`}><span className="text_button !text-white">{isSaving ? 'Procesando...' : t('summary.proceed_to_payment')}</span></button></div>
            </div>
          </div>
        )}
      </div>
